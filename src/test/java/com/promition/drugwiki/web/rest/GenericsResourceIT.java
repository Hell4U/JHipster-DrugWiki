...
class GenericsResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/generics";
    ...
    /**
     * This repository is mocked in the com.promition.drugwiki.repository.search test package.
     *
     * @see com.promition.drugwiki.repository.search.GenericsSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createGenerics() throws Exception {
        ...
        // Validate the Generics in Elasticsearch
        verify(mockGenericsSearchRepository, times(1)).save(testGenerics);
    }
    ...
    void createGenericsWithExistingId() throws Exception {
        ...
        // Validate the Generics in Elasticsearch
        verify(mockGenericsSearchRepository, times(0)).save(generics);
    }
    ...
    void putNewGenerics() throws Exception {
        ...
        // Validate the Generics in Elasticsearch
        verify(mockGenericsSearchRepository).save(testGenerics);
    }
    ...
    void deleteGenerics() throws Exception {
        ...
        // Validate the Generics in Elasticsearch
        verify(mockGenericsSearchRepository, times(1)).deleteById(generics.getId());
    }

    @Test
    @Transactional
    void searchGenerics() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        genericsRepository.saveAndFlush(generics);
        when(mockGenericsSearchRepository.search(queryStringQuery("id:" + generics.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(generics), PageRequest.of(0, 1), 1));

        // Search the generics
        restGenericsMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + generics.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(generics.getId().intValue())))
            .andExpect(jsonPath("$.[*].gname").value(hasItem(DEFAULT_GNAME)))
