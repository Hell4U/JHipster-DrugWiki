...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_COMPANY: 'company/PARTIAL_UPDATE_COMPANY',
...
export default (state: CompanyState = initialState, action): CompanyState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_COMPANY):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_COMPANY):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_COMPANY):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const partialUpdate: ICrudPutAction<ICompany> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_COMPANY,
