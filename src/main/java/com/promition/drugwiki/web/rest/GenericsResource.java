...
public class GenericsResource {
    ...
    /**
     * {@code SEARCH  /_search/generics?query=:query} : search for the generics corresponding
     * to the query.
     *
     * @param query the query of the generics search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/generics")
    public ResponseEntity<List<GenericsDTO>> searchGenerics(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Generics for query {}", query);
        Page<GenericsDTO> page = genericsService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
